...
import com.qtec.src.field.gt.GTFiniteElement;
import com.qtec.src.field.gt.GTFiniteField;
import com.qtec.src.pairing.f.map.AbstractPairingMap;
...
public class TypeFRatePairingMap extends AbstractPairingMap {
    ...
    public Element pairing(Point P, Point Q) {
        ...
        t.add(Q22);
        BigInteger subtract = q.pow(8).add(q.pow(6)).subtract(q.pow(2)).subtract(BigInteger.ONE);
        long begin = System.currentTimeMillis();
        // 下面操作耗时
        Element e = f.duplicate().pow(subtract).pow(pairingdata.tateExp);
        System.err.println("###-pairing中e的计算耗时：" + (System.currentTimeMillis() - begin));
    ...
    public Element line(Point A, Point B, Point C) {
        ...
        Element cof5 = lamda.duplicate().mulZn(C.getX().duplicate());
        ...
        tempfp2.getX().set(C.getY().duplicate().negate());
